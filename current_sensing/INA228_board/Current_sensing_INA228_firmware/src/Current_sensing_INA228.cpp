...
INA228::INA228(uint8_t i2cAddr, float shuntOhms, float maxCurrentA, TwoWire &wirePort)
    : wire(wirePort),
      ...
      currentLSB(0.0f),
      shuntCal(0),
      adcRangeHigh(false) // default to high range (±163.84mV)
{
}
...
void INA228::computeCalibration()
{
    ...
    currentLSB = maxCurrent / (float)(1UL << 19); // A per LSB
...
bool INA228::readCurrent(float &currentA)
{
    ...
    Serial.println(s20, DEC);
    Serial.println(currentLSB, 10);
    Serial.println(maxCurrent, 10);
